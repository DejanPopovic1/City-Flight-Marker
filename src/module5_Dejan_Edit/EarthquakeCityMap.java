...
public class EarthquakeCityMap extends PApplet {
	...
	private Marker lastSelected = null;
	...
	public void mouseClicked() {
		...
		if (lastClicked == null) {
			...
			lastClicked = null;
			unhideMarkers();
			// System.out.print("lastUnclicked");
		}
	}

	public void checkIfEarthquakeIsClicked() {
		for (Marker earthquake : quakeMarkers) {
			if (earthquake.isInside(map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) earthquake;
				hideRemainingEarthquakes(lastClicked);
				showAffectedCities(lastClicked);
				// System.out.print("lastClicked");
			}
		}
	}

	public void checkIfCityIsClicked() {
		for (Marker city : cityMarkers) {
			if (city.isInside(map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) city;
				hideRemainingCities(lastClicked);
				showThreatEarthquakes(lastClicked);
				// System.out.print("lastClicked");
			}
		}
	}

	public void hideRemainingCities(CommonMarker clickedCity) {
		for (Marker city : cityMarkers) {
			city.setHidden(true);
		}
		clickedCity.setHidden(false);
	}

	public void showThreatEarthquakes(CommonMarker clickedCity) {
		for (Marker earthquake : quakeMarkers) {
			if (isCityInThreatCircle(clickedCity, earthquake))
	...
	public boolean isCityInThreatCircle(CommonMarker clickedCity, Marker earthquake){
		double distanceBetweenCityAndEarthquake = earthquake.getDistanceTo(clickedCity.getLocation());
		double threatDistance = ((EarthquakeMarker) earthquake).threatCircle();
		return (distanceBetweenCityAndEarthquake <= threatDistance);
	}
	
	public void showAffectedCities(CommonMarker clickedEarthquake) {
		for (Marker city : cityMarkers) {
			if (isCityInThreatCircle((CommonMarker) city, clickedEarthquake)) {
	...
	public void hideRemainingEarthquakes(CommonMarker clickedEarthquake) {
		for (Marker earthquake : quakeMarkers) {
			earthquake.setHidden(true);
		}
	...
	/*
	 * for (Marker earthquake : quakeMarkers) { if (earthquake == lastClicked) {
	 * earthquake.setHidden(false); } else if (earthquake != lastClicked) {
	 * earthquake.setHidden(true); } }
	 * 
	 * }
	 */
